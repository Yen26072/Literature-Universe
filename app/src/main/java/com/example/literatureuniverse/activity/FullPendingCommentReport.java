...
public class FullPendingCommentReport extends BaseActivity {
    DatabaseReference commentReportRef;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_full_pending_comment_report);
        ...
        recyclerView = findViewById(R.id.recyclerCommentReport);
...
//        reportCommentAdapter = new ReportCommentAdapter(this, new ArrayList<>());
        ...
        reportCommentAdapter = new ReportCommentAdapter(this, new ArrayList<>(), report -> {
            Intent intent = new Intent(FullPendingCommentReport.this, DetailCommentReport.class);
...
//        showListPendingCommentReport();
    }

    @Override
    protected void onResume() {
        super.onResume();
        showListPendingCommentReport();   // load lại danh sách mỗi lần quay lại
    }

    private void showListPendingCommentReport() {
        commentReportRef.orderByChild("status").equalTo("pending")
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        for (DataSnapshot ds : snapshot.getChildren()) {
                            CommentReport report = ds.getValue(CommentReport.class);
                            if (report != null) {
                                commentReportList.add(report);
                            }
                        }

                        Collections.reverse(commentReportList);
                        currentPage = 1;
                        updatePagination();
    ...
    private void updatePagination() {
        totalPages = (int) Math.ceil((double) commentReportList.size() / itemsPerPage);
    ...
    private void displayCurrentPage() {
        ...
        int end = Math.min(start + itemsPerPage, commentReportList.size());

        Log.d("MyStoryDebug", "Hiển thị từ index " + start + " đến " + (end - 1));

        List<CommentReport> subList = commentReportList.subList(start, end);
        reportCommentAdapter.setData(subList);
    }
}