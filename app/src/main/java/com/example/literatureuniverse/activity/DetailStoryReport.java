...
public class DetailStoryReport extends BaseActivity {
    private ImageView imgReporter, imgCoverStory;
    private TextView tvReporter, tvReason, tvTime, tvStoryName, tvTitleChapter, tvChapterContent;
    private Button btnConfirmViolation, btnConfirmNoViolation, btnRejected, btnAccepted;

    private String reportId, chapterId, storyId;
    private StoryReport report;
    private Boolean isAppeal;

    private DatabaseReference reportsRef, usersRef, storiesRef, chaptersRef;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_detail_story_report);
        ...
        if (reportId == null || chapterId == null || storyId == null) {
    ...
    private void rejectAppeal(){
        ...
        reportRef.get().addOnSuccessListener(snapshot -> {
            ...
            StoryReport report = snapshot.getValue(StoryReport.class);
            if (report == null) return;

            String punishedUserId = report.getStoryOwnerId();  // người bị xử phạt
            String violationId = report.getViolationId();
            String chapterId = report.getChapterId();
            String storyId = report.getStoryId();
            String reporter = report.getReporterId();
            ...
            userRef.get().addOnSuccessListener(userSnap -> {
                ...
                String reportType = "story";
                String message = "Đã TỪ CHỐI khiếu nại – Giữ nguyên toàn bộ hình phạt!";

                // 6) Gửi thông báo kết quả khiếu nại
                sendPunishNotification(punishedUserId, reportId, reporter, storyId, chapterId, reportType, message);
    ...
    private void rejectReport() {
        ...
        reportsRef.child(report.getReportId()).updateChildren(updates)
                .addOnSuccessListener(aVoid -> {
                    ...
                    sendNotificationToReporter(report.getReportId(),report.getStoryId(), chapterId,
    ...
    private void sendNotificationToReporter(String reportId,String storyId, String chapterId, String reporterId, String title, String message) {
        ...
        Notification noti = new Notification(
                ...
                "story",
                storyId,
                chapterId,    // có thể null nếu comment ở review
                null,           //commentId
                null,         // reviewId null
    ...
    private void loadReportData() {
        reportsRef.child(reportId).get().addOnSuccessListener(snapshot -> {
            report = snapshot.getValue(StoryReport.class);
            if (report == null) {
            ...
            //Load story
            storiesRef.child(storyId).get().addOnSuccessListener(storySnap ->{
                if(storySnap.exists()){
    ...
    private void applyPenalty(AppRule rule) {
        long now = System.currentTimeMillis();
        String punishedUserId = report.getStoryOwnerId();   // người bị phạt
        // 1. Đánh dấu comment là đã bị xoá (ẩn khỏi người dùng)
        storiesRef.child(report.getStoryId()).child("deleted").setValue(true);
        // 2. Phạt user commentOwner
        usersRef.child(report.getStoryOwnerId()).get().addOnSuccessListener(snapshot -> {
    ...
    private void sendPunishNotification(String punishedUserId, String reportId, String reporter, String storyId, String chapterId, String reportType, String message) {
        DatabaseReference notifRef = FirebaseDatabase.getInstance()
                .getReference("notifications")
                .child(punishedUserId);
