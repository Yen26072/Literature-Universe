...
public class DetailReviewReport extends BaseActivity {
    private ImageView imgReporter, imgReviewAuthor;
    private TextView tvReporter, tvReason, tvTime, tvReviewAuthor, tvReviewContent;
    ...
    private DatabaseReference reportsRef, usersRef, reviewsRef;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detail_review_report);
        ...
        imgReviewAuthor = findViewById(R.id.imgCommentAuthor);
        tvReporter = findViewById(R.id.tvReporter);
        tvReason = findViewById(R.id.tvReason);
        tvTime = findViewById(R.id.tvTime);
        tvReviewAuthor = findViewById(R.id.tvCommentAuthor);
        tvReviewContent = findViewById(R.id.tvCommentContent);
    ...
    private void rejectAppeal(){
        ...
        reportRef.get().addOnSuccessListener(snapshot -> {
            ...
            ReviewReport report = snapshot.getValue(ReviewReport.class);
            if (report == null) return;

            String punishedUserId = report.getReviewOwnerId();  // người bị xử phạt
            ...
            userRef.get().addOnSuccessListener(userSnap -> {
                ...
                sendPunishNotification(punishedUserId, reportId, reviewId, storyId, reportType, message);
    ...
    private void rejectReport() {
        ...
        reportsRef.child(report.getReportId()).updateChildren(updates)
                .addOnSuccessListener(aVoid -> {
                    ...
                    sendNotificationToReporter(report.getReportId(),report.getStoryId(), report.getReviewId(),
    ...
    private void sendNotificationToReporter(String reportId,String storyId,String reviewId, String reporterId, String title, String message) {
    ...
    private void loadReportData() {
        reportsRef.child(reportId).get().addOnSuccessListener(snapshot -> {
            report = snapshot.getValue(ReviewReport.class);
            if (report == null) {
            ...
            // Load comment info
            reviewsRef.child(report.getReviewId()).get().addOnSuccessListener(reviewSnap -> {
                if (reviewSnap.exists()) {
                    String reviewContent = reviewSnap.child("content").getValue(String.class);
                    ...
                    usersRef.child(userId).get().addOnSuccessListener(authorSnap -> {
                        ...
                        if (author != null) {
                            tvReviewAuthor.setText(author.getUsername());
                            Glide.with(this).load(author.getAvatarUrl()).circleCrop().into(imgReviewAuthor);
                        }
                    });
                } else {
                    tvReviewContent.setText("Comment đã bị xóa");
    ...
    private void applyPenalty(AppRule rule) {
        long now = System.currentTimeMillis();
        String punishedUserId = report.getReviewOwnerId();   // người bị phạt
        // 1. Đánh dấu comment là đã bị xoá (ẩn khỏi người dùng)
        reviewsRef.child(report.getReviewId()).child("deleted").setValue(true);
        // 2. Phạt user commentOwner
        usersRef.child(report.getReviewOwnerId()).get().addOnSuccessListener(snapshot -> {
    ...
    private void sendPunishNotification(String punishedUserId, String reportId, String reviewId, String storyId, String reportType, String message) {
        DatabaseReference notifRef = FirebaseDatabase.getInstance()
                .getReference("notifications")
                .child(punishedUserId);
        ...
        Notification noti = new Notification(
                ...
                null,
                reviewId,           // reviewId nếu report review
                System.currentTimeMillis(),
                false,          // chưa đọc
                message
        );
