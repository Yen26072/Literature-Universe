...
public class FullCompletedStories extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_full_completed_stories);
        ...
        showListCompletedStory();
    }

    private void showListCompletedStory() {
        storyRef.orderByChild("updatedAt")
    ...
    private void updatePagination() {
        totalPages = (int) Math.ceil((double) completedStories.size() / itemsPerPage);
    ...
    private void displayCurrentPage() {
        ...
        int end = Math.min(start + itemsPerPage, completedStories.size());

        Log.d("MyStoryDebug", "Hiển thị từ index " + start + " đến " + (end - 1));

        List<Story> subList = completedStories.subList(start, end);
